...
```
prefix: 무엇을 했는지 (한 줄, 50자 내외)

왜 그렇게 했는지. 무엇을 바꿨는지는 diff 를 보면 아니까,
diff 만 봐서는 알 수 없는 배경·판단 근거를 적는다.
본문은 필요할 때만. 한 줄로 충분하면 제목만 써도 된다.
```

**규칙**

- **한국어로 쓴다.** PR 제목·본문도 한국어. (초기 커밋 일부가 영어인데, 그건 규칙을 정하기 전에 쓴 것이다)
- prefix 는 브랜치명과 같은 목록을 쓴다 — `feat` `fix` `docs` `style` `refactor` `test` `chore` `hotfix`
- 제목 끝에 **마침표를 찍지 않는다.**
- 항목을 나열해야 하면 제목에 `—` 로 붙인다. 예: `docs: 리뷰 반영 — 시점 1인칭 명시, TODO 2건 추가`
- **한 커밋에 한 가지 변경.** 성격이 다른 변경은 커밋을 나눈다.
- 되돌릴 땐 `git revert` 를 쓰고, 메시지에 **왜 되돌리는지**를 남긴다.

**제목만 보고 무슨 일이 있었는지 알 수 있게 쓴다**

| | |
|---|---|
| ❌ | `docs: 리뷰 반영` — 무엇을 어떻게 고쳤는지 안 보임 |
| ❌ | `fix: 버그 수정` / `chore: 수정` — 아무 정보가 없음 |
| ✅ | `docs: 리뷰 반영 — 시점 1인칭 명시, TODO 2건 추가` |
| ✅ | `fix: force push 가드레일이 뚫리던 구멍 수정` |
| ✅ | `feat: 제한된 객실 조사 시스템 추가` |

**본문을 쓰면 좋은 경우**

- 왜 이 방식을 골랐는지 설명이 필요할 때 (특히 다른 선택지를 버린 이유)
- 리뷰 지적을 반영한 커밋 — 무엇을 지적받아 어떻게 고쳤는지
- 검증을 했다면 그 결과 (예: 어떤 입력이 차단되고 어떤 게 통과하는지)
- 나중에 "이거 왜 이렇게 돼 있지?" 소리가 나올 것 같은 변경
...
- `dismiss_stale_reviews` **꺼짐** (2026-09-15 변경) — 승인을 받은 뒤 커밋을 더 올려도 승인이 유지된다

> `dismiss_stale_reviews`를 켜면 새 커밋이 올라올 때마다 기존 승인이 자동으로 취소된다.
> 처음엔 켜뒀는데, 리뷰 반영 커밋을 올릴 때마다 승인이 날아가서 문서 오타 하나에도 재승인을 받아야 했다.
> **다시 켜려면 그 마찰을 감수할 이유가 있는지 먼저 확인할 것** — 승인 후 검토 없이 커밋이 들어가는 걸
> 막고 싶다면, 전체를 켜는 대신 코드 디렉터리에만 CODEOWNERS를 거는 방법도 있다.
